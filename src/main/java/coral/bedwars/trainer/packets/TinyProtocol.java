package coral.bedwars.trainer.packets;
...
import coral.bedwars.trainer.packets.Reflection.MethodInvoker;
import coral.bedwars.trainer.packets.Reflection.FieldAccessor;
...
public abstract class TinyProtocol {
	...
	private void registerChannelHandler() {
		Object mcServer = getMinecraftServer.from(Bukkit.getServer());
		Object serverConnection = getServerConnection.from(mcServer);
		...
		for (int i = 0; looking; i++) {
			List<Object> list = Reflection.getField(serverConnection.getClass(), List.class, i).from(serverConnection);
	...
	public Channel getChannel(Player player) {
		...
		if (channel == null) {
			Object connection = getConnection.from(getPlayerHandle.invoke(player));
			Object manager = getManager.from(connection);

			channelLookup.put(player.getName(), channel = getChannel.from(manager));
		}
	...
	private final class PacketInterceptor extends ChannelDuplexHandler {
		...
		private void handleLoginStart(Channel channel, Object packet) {
			if (PACKET_LOGIN_IN_START.isInstance(packet)) {
				GameProfile profile = getGameProfile.from(packet);
